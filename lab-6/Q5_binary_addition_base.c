...
int binaryAddition(int operand_1, int operand_2) {
  ...
  while (operand_2 != 0) {
    ...
    // Calculate carry: bitwise AND operation followed by a left shift
    carry = (operand_1 & operand_2) << 1;
    printf("For addition operation: carry = %d\n", carry);

    // Calculate sum at each bit position using XOR
    sum = operand_1 ^ operand_2;
    printf("For addition operation: sum = %d\n", sum);

    // Update operand_1 with the new sum
    operand_1 = sum;
    printf("For addition operation: operand_1 = %d\n", operand_1);

    // Update operand_2 with the new carry value
    operand_2 = carry;
    printf("For addition operation: operand_2 = %d\n", operand_2);
  }
...
int binarySubtracton(int operand_1, int operand_2) {
  ...
  // Step 1 for (-operand_2): finding two's complement for operand_2 =
  // ~operand_2 + 1
  operand_2 = binaryAddition(~operand_2, 1);
  return binaryAddition(operand_1, operand_2);
  // // Step 2 for operand_1 + (-operand_2): perform addition of two binary
  // // operands Iterate till there is no carry
  // while (operand_2 != 0) {
  //   //// Please do not change the codes above ////
  //   //// Please add your codes below ////

  //   // calculating the carry by bitwise AND operation, then left shift 1 bit
  //   carry = (operand_1 & operand_2) << 1;  // please add your code in this
  //   line
